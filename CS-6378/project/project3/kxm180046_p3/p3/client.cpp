...
int main(void)
{
	...
	getIPandPortNeighbour(serv_neighbours, n_serv_neigh, "res/s.cfg");

	// debug 
	printALLNeighbour(serv_neighbours, n_serv_neigh, IP, PORT);

    // get the ip of the client neighbours
    string cfg = "res/c" + to_string(clientID) + ".cfg";
    printf("Client information\n");
	getIPandPortNeighbour(client_neighbours, n_client_neigh, cfg);
    ...
    //cout << clientWork('R', serv_neighbours, n_serv_neigh, client_neighbours, n_client_neigh, files[0], "")[0] << endl;
    ...
    for (int trial = 0; trial < 10; trial++)
    {
        ...
        if (random_trial_read)
        {
            // READ(filename) -> string
            cout << "READ file:" << files[random_num] << endl;
